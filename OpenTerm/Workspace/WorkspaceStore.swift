...
final class WorkspaceStore: ObservableObject {
	...
	func queueRemoteToolAudit(on profile: SSHProfileSummary) {
		...
		if profile.platform == .windows {
			script = #"powershell -NoProfile -Command "Write-Host 'OpenTerm Windows SSH audit'; Get-ComputerInfo -Property OsName,OsVersion,CsName; Write-Host ''; Write-Host 'Developer tools:'; foreach ($tool in 'git','python','py','node','npm','yarn','pnpm','tsc','ts-node','deno','bun','go','rustc','cargo','ruby','php','java','javac','perl','lua','dotnet','docker','winget','powershell','mvn','gradle','kotlinc','scala','erl','elixir','ghc','cabal','stack','zig','dart','julia','R','Rscript','swift') { $cmd = Get-Command $tool -ErrorAction SilentlyContinue; if ($cmd) { Write-Host ($tool + ': ' + $cmd.Source) } else { Write-Host ($tool + ': missing') } }""#
		} else {
			script = #"printf 'OpenTerm remote runtime audit\n'; uname -a 2>/dev/null; printf '\nPackage managers:\n'; for tool in apt apt-get apk dnf yum brew pkg pacman zypper; do command -v $tool >/dev/null 2>&1 && echo "$tool: $(command -v $tool)"; done; printf '\nDeveloper runtimes/tools:\n'; for tool in git python3 pip3 node npm yarn pnpm tsc ts-node deno bun go rustc cargo ruby php java javac perl lua gcc g++ clang make cmake sqlite3 docker htop nano vim tmux dotnet mvn gradle kotlinc scala erl elixir ghc cabal stack zig dart julia R Rscript swift; do if command -v $tool >/dev/null 2>&1; then printf '%s: ' $tool; $tool --version 2>&1 | head -n 1; else echo "$tool: missing"; fi; done"#
		}
		openTerminal(command: sshCommand(for: profile, remoteCommand: script, batchMode: false), executeNow: true)
		touchProfile(profile.id)
	...
	private func remoteDevStackScript(for flavor: String) -> String {
		let linuxRuntimeCommands = "git python3 pip3 node npm yarn pnpm tsc ts-node deno bun go rustc cargo ruby php java javac perl lua gcc g++ clang make cmake sqlite3 docker htop nano vim tmux dotnet mvn gradle kotlinc scala erl elixir ghc cabal stack zig dart julia R Rscript swift"
		let verifyLinux = """
		printf '\nInstalled runtime versions:\n' ; for tool in \(linuxRuntimeCommands); do if command -v $tool >/dev/null 2>&1; then printf '%s: ' $tool; $tool --version 2>&1 | head -n 1; else echo "$tool: missing"; fi; done
		"""
		let verifyWindows = #"Write-Host ''; Write-Host 'Installed runtime versions:'; foreach ($tool in 'git','python','node','npm','yarn','pnpm','tsc','ts-node','deno','bun','go','rustc','cargo','ruby','php','java','javac','perl','lua','dotnet','docker','mvn','gradle','kotlinc','scala','erl','elixir','ghc','cabal','stack','zig','dart','julia','R','Rscript','swift') { $cmd = Get-Command $tool -ErrorAction SilentlyContinue; if ($cmd) { Write-Host ($tool + ': ' + $cmd.Source) } else { Write-Host ($tool + ': missing') } }"#
		let npmTools = "typescript ts-node yarn pnpm"
		let debianCore = "git python3 python3-pip nodejs npm golang rustc cargo ruby-full php-cli default-jdk perl lua5.4 build-essential clang cmake sqlite3 docker.io htop nano vim tmux r-base maven gradle"
		let debianOptional = "yarnpkg kotlin scala elixir erlang ghc cabal-install haskell-stack zig julia dotnet-sdk-8.0 swiftlang swift deno bun dart"
		let alpineCore = "git python3 py3-pip nodejs npm go rust cargo ruby php php-cli openjdk17 perl lua5.4 build-base clang cmake sqlite docker htop nano vim tmux R maven gradle"
		let alpineOptional = "yarn pnpm deno bun kotlin scala elixir erlang ghc cabal stack zig dart julia dotnet6-sdk swift"
		let fedoraCore = "git python3 python3-pip nodejs npm golang rust cargo ruby php-cli java-17-openjdk-devel perl lua gcc gcc-c++ clang make cmake sqlite docker htop nano vim tmux R maven gradle"
		let fedoraOptional = "yarnpkg deno bunjs kotlin scala elixir erlang ghc cabal-install stack zig julia dotnet-sdk-8.0 swift-lang dart"
		let npmInstall = "if command -v npm >/dev/null 2>&1; then sudo npm install -g \(npmTools) || echo 'optional npm global tools unavailable'; fi"

		switch flavor {
		case "Debian/Ubuntu":
			return "set -e; sudo apt update; sudo apt install -y \(debianCore); for pkg in \(debianOptional); do sudo apt install -y $pkg || echo \"optional package unavailable: $pkg\"; done; \(npmInstall); \(verifyLinux)"
		case "Alpine":
			return "set -e; sudo apk add --no-cache \(alpineCore); for pkg in \(alpineOptional); do sudo apk add --no-cache $pkg || echo \"optional package unavailable: $pkg\"; done; \(npmInstall); \(verifyLinux)"
		case "Fedora/RHEL":
			return "set -e; if command -v dnf >/dev/null 2>&1; then sudo dnf install -y \(fedoraCore); for pkg in \(fedoraOptional); do sudo dnf install -y $pkg || echo \"optional package unavailable: $pkg\"; done; else sudo yum install -y \(fedoraCore); for pkg in \(fedoraOptional); do sudo yum install -y $pkg || echo \"optional package unavailable: $pkg\"; done; fi; \(npmInstall); \(verifyLinux)"
		case "Windows":
			return """
			powershell -NoProfile -ExecutionPolicy Bypass -Command "if (-not (Get-Command winget -ErrorAction SilentlyContinue)) { Write-Error 'winget is required for Windows runtime install'; exit 1 }; $packages=@('Git.Git','Python.Python.3.12','OpenJS.NodeJS','Yarn.Yarn','pnpm.pnpm','DenoLand.Deno','Oven-sh.Bun','GoLang.Go','Rustlang.Rustup','RubyInstallerTeam.Ruby','PHP.PHP','EclipseAdoptium.Temurin.21.JDK','StrawberryPerl.StrawberryPerl','Microsoft.DotNet.SDK.8','Docker.DockerDesktop','Apache.Maven','Gradle.Gradle','JetBrains.Kotlin','Scala.Scala','Erlang.Erlang','Elixir.Elixir','Haskell.HaskellStack','zig.zig','Dart.Dart','Julialang.Julia','RProject.R','Swift.Toolchain'); foreach ($package in $packages) { winget install --accept-source-agreements --accept-package-agreements --silent --id $package -e; if ($LASTEXITCODE -ne 0) { Write-Warning ('Install failed or unavailable: ' + $package) } }; \(verifyWindows)"
			"""
		default:
			return "set -e; if command -v apt-get >/dev/null 2>&1; then sudo apt update && sudo apt install -y \(debianCore); for pkg in \(debianOptional); do sudo apt install -y $pkg || echo \"optional package unavailable: $pkg\"; done; elif command -v apk >/dev/null 2>&1; then sudo apk add --no-cache \(alpineCore); for pkg in \(alpineOptional); do sudo apk add --no-cache $pkg || echo \"optional package unavailable: $pkg\"; done; elif command -v dnf >/dev/null 2>&1; then sudo dnf install -y \(fedoraCore); for pkg in \(fedoraOptional); do sudo dnf install -y $pkg || echo \"optional package unavailable: $pkg\"; done; elif command -v yum >/dev/null 2>&1; then sudo yum install -y \(fedoraCore); for pkg in \(fedoraOptional); do sudo yum install -y $pkg || echo \"optional package unavailable: $pkg\"; done; else echo 'No supported package manager found. Use a Linux distro with apt/apk/dnf/yum or run the Windows kit on a winget-enabled host.'; exit 1; fi; \(npmInstall); \(verifyLinux)"
		}
	}
