...
- SSH profiles include remote Runtime Audit and Runtime Kit actions that verify package managers/tool versions, then queue package-manager commands for Git, Python 3/pip, Node.js/npm, Go, Rust/Cargo, Ruby, PHP, Java/JDK, Perl, Lua, C/C++ build tools, Clang, CMake, SQLite, Docker, htop, nano, vim, and tmux on Debian/Ubuntu, Alpine, Fedora/RHEL, auto-detected Linux hosts, or winget-enabled Windows hosts.
...
- Package managers such as `apt`, `brew`, `pkg`, `apk`, `dnf`, `winget`, and `npm` are not available locally unless their runtimes are compiled into the app, embedded as app resources, or run on a remote machine. OpenTerm now treats broad runtime installs as SSH remote Runtime Kit actions, not local iPhone package-manager magic.
