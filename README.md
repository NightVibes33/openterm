...
- Legacy OpenTerm terminal core is still present and embedded inside a stronger SwiftUI terminal workspace shell with a command deck, terminal-window framing, honest local/remote runtime labels, and quick navigation to Files, Servers, AI, and Settings. Default terminal colors now use a high-contrast modern palette instead of the old app-tint-on-panel look. Several older storyboard/XIB scripting and documentation panels still exist and are tracked as modernization debt rather than finished 2026-quality UI, but the Scripts panel remains functional and the bundled examples are maintained as runnable script samples.
...
- SSH profiles persist locally with label, host, username, port, host OS, auth type, key path, startup path, notes, and last-used timestamps; password profiles store the password in Keychain and copy it before opening an interactive SSH login prompt. New installs no longer seed fake example hosts, and blank profiles are rejected.
- SSH quick connect now builds and runs real `ssh` commands through the terminal instead of only switching screens.
- SSH profiles include remote Runtime Audit and Full Runtime Kit actions that verify package managers/tool versions, then queue package-manager commands for Git, Python 3/pip, Node.js/npm/Yarn/pnpm/TypeScript, Deno, Bun, Go, Rust/Cargo, Ruby, PHP, Java/JDK, Perl, Lua, .NET, C/C++ build tools, Clang, CMake, SQLite, Docker, Maven, Gradle, Kotlin, Scala, Erlang/Elixir, Haskell, Zig, Dart, Julia, R, Swift where available, htop, nano, vim, and tmux on Debian/Ubuntu, Alpine, Fedora/RHEL, auto-detected Linux hosts, or winget-enabled Windows hosts. Optional distro packages are best-effort so one unavailable runtime does not abort the whole kit.
...
- The Files tab now works as a real iOS document workspace with a more native file-manager UI: Apple document-browser access, visible import/new/refresh actions, folder/file counts, folder navigation, Files app import, local search, file-kind metadata, create file/folder, rename, duplicate, copy path, open terminal here, delete, file share/export, folder tar archive export, UTF-8 editing, syntax highlighting, search/match highlighting, lightweight completion chips, diff view against the opened version, and save back to disk.
...
- Server monitors persist locally, poll Linux CPU/memory/disk/load snapshots over noninteractive SSH, support Windows 11/Windows Server polling through PowerShell/CIM for key/agent profiles, and keep a local acknowledgeable alert history when thresholds change; monitors are user-created and monitor cards appear only after real poll results.
...
- Legacy Pods are still present because the terminal, scripts, input assistant, and Cub editor paths still depend on them. The Podfile now normalizes vendored Pods to the iOS 18 floor, Swift 5, no bitcode, and non-active-arch-only builds. TabView vendored sources have also been patched from pre-Swift-4 UIKit child-controller/deceleration APIs to modern Swift 5 UIKit names, InputAssistant now uses modern collection-view sizing constants, and PanelKit now uses modern collection first-index APIs, so old framework modernization is enforced by CI instead of hidden by old Swift settings. The immediate modernization path is to keep the working terminal stack compiling, replace visible legacy UI with SwiftUI/native screens, then delete Pods one by one only after their imports are gone.
...
- Package managers such as `apt`, `brew`, `pkg`, `apk`, `dnf`, `winget`, and `npm` are not available locally unless their runtimes are compiled into the app, embedded as app resources, or run on a remote machine. OpenTerm now treats broad runtime installs as SSH remote Full Runtime Kit actions, not local iPhone package-manager magic. The kit audits every target after install and leaves unsupported distro packages marked as missing instead of pretending they exist.
...
- Local runtimes like Git, Python, Node, Deno, Bun, Go, Rust, Java, .NET, Vim, Nano, Tmux, or Htop should be treated as future embedded-toolchain work, with licensing, architecture, sandbox paths, and binary size reviewed per tool. The current implementation targets real SSH hosts first.
